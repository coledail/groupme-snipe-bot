...
function createGameRepository(db) {
  ...
  return {
    ...
    async createActive(name) {
      if (isPg) {
        return await withTransaction(db, async (client) => {
          await client.query('UPDATE games SET active = false WHERE active = true');
          const res = await client.query('INSERT INTO games (name, active) VALUES ($1, true) RETURNING *', [name]);
          return toGame(res.rows[0]);
        });
      }
      db.exec('BEGIN');
      try {
        ...
        const game = toGame(findByIdStmt.get(result.lastInsertRowid));
        db.exec('COMMIT');
        return game;
      } catch (err) {
        db.exec('ROLLBACK');
    ...
    async activate(gameId) {
      if (isPg) {
        return await withTransaction(db, async (client) => {
          const existing = await client.query('SELECT * FROM games WHERE id = $1', [gameId]);
          ...
          await client.query('UPDATE games SET active = false WHERE active = true');
          await client.query('UPDATE games SET active = true WHERE id = $1', [gameId]);
      ...
      try {
        ...
        const activatedGame = toGame(findByIdStmt.get(gameId));
        db.exec('COMMIT');
